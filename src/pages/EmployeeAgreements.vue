<template>
  <page-container title="Çalışan İş Sözleşmeleri">
    <template #icon>
    ...
    <q-page-container>
      ...
      <div class="q-pa-md">
        ...
        <q-table :grid="$q.screen.xs" flat bordered title="Çalışan İş Sözleşme Listesi" ref="tableRef" :rows="data"
  ...
  -->
  <form-modal-dialog v-model="addUpdateDialog" title="İşyeri Sözleşmesi" icon="fa-solid fa-briefcase" negative="Kapat"
    @positive="updateCommit()">

    <div class="input-group">
      <q-select class="txt" filled v-model="dataItem.nameSurname" :options="['Seçiniz', 'Ahmet', 'Buğra', 'Ali']"
        label="Adı Soyadı" lazy-rules :rules="[val => val && val.length > 0 || 'Boş geçilemez']" />
    </div>
...
import data from 'src/data/employeeAgreements.json'
...
const columns = [
  ...
  { name: 'nameSurname', label: 'Adı Soyadı', align: 'left', field: 'nameSurname', sortable: true },
