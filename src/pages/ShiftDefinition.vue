<template>
  <page-container title="Vardiya Tanımlama">
      ...
      <div>
        ...
        <div class="md-screen">
          <div class="q-ma-md div-table">
            ...
            <div class="row">
              ...
              <div class="col text-left text-bold">Vardiya Adı / Açıklama</div>
              <div class="col text-left text-bold">Başlangıç Tarihi</div>
              <div class="col text-left text-bold">Bitiş Tarihi</div>
            ...
            <div class="row div-row" v-for="(i, index) in data" :key="i" :class="GetRowClass(index)">
              ...
              <div class=" col text-left"> {{ i.shiftName }} </div>
              <div class=" col text-left">{{ i.startDate }}</div>
              <div class=" col text-left">{{ i.endDate }}</div>
              <div class=" col text-left">
        ...
        <div class="sm-screen">
          ...
          <div class="sm-card" v-for="i in data" :key="i">
            ...
            <div class="sm-row">
              <div class="sm-title">Vardiya Adı / Açıklama</div>
              <div class="sm-content">{{ i.shiftName }}</div>
            </div>

            <div class="sm-row">
              <div class="sm-title">Başlangıç Tarigi</div>
              <div class="sm-content">{{ i.startDate }}</div>
            </div>

            <div class="sm-row">
              <div class="sm-title">Bitiş Tarihi</div>
              <div class="sm-content">{{ i.endDate }}</div>
            </div>
  ...
  -->
  <form-modal-dialog v-model="addUpdateDialog" title="Vardiya" icon="fa-solid fa-briefcase" negative="Kapat"
    @positive="updateCommit()">



    <div class="input-group">
      <q-input class="txt" v-model="dataItem.shiftName" filled label="Vardiya Adı / Açıklama" lazy-rules
        :rules="[val => val && val.length > 0 || 'Boş geçilemez']" />
    </div>

    <div class="input-group">
      <q-input class="txt" type="date" v-model="dataItem.startDate" filled label="Başlangıç Tarihi" lazy-rules
        :rules="[val => val && val.length > 0 || 'Boş geçilemez']" />
    </div>

    <div class="input-group">
      <q-input class="txt" type="date" v-model="dataItem.endDate" filled label="Bitiş Tarihi" lazy-rules
        :rules="[val => val && val.length > 0 || 'Boş geçilemez']" />
    </div>
...
import data from 'src/data/shiftDefinition.json'
