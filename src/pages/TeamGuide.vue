<template>
  <page-container title="Grup/Ekip Rehberi">
        ...
        <div class="md-screen">
          <div class="q-ma-md div-table">
            ...
            <div class="row">
              ...
              <div class="col text-left text-bold">Grup/Ekip Adı</div>
              <div class="col text-left text-bold">Açıklama</div>
            ...
            <div class="row div-row" v-for="(i, index) in data" :key="i" :class="GetRowClass(index)">
              ...
              <div class=" col text-left"> {{ i.teamName }} </div>
              <div class=" col text-left"> {{ i.explanation }} </div>
              <div class=" col text-left">
        ...
        <div class="sm-screen">
          ...
          <div class="sm-card" v-for="i in data" :key="i">
            ...
            <div class="sm-row">
              ...
              <div class="sm-content">{{ i.teamName }}</div>
            </div>

            <div class="sm-row">
              <div class="sm-title">Açıklama</div>
              <div class="sm-content">{{ i.explanation }}</div>
            </div>
  ...
  -->
  <form-modal-dialog v-model="addUpdateDialog" title="Grup/Ekip" icon="fa-solid fa-briefcase" negative="Kapat"
    @positive="updateCommit()">

    <div class="input-group">
      <q-select class="select" v-model="dataItem.teamName" filled :virtual-scroll-horizontal="false"
        :options="['Seçiniz', 'Arge 360', 'Garanti Bankası', 'İş Bankası']" label="Grup/Ekip" lazy-rules
        :rules="[val => val && val.length > 0 || 'Boş geçilemez']" />
    </div>

    <div class="input-group">
      <q-input class="txt" type="textarea" v-model="dataItem.explanation" filled label="Açıklama" lazy-rules
        :rules="[val => val && val.length > 0 || 'Boş geçilemez']" />
    </div>
  ...
  <!-- Delete modal start -->
  <form-modal-dialog v-model="deleteDialog" title="Ekip Silme Onayı" icon="fa-solid fa-trash" @positive="del()"
    positive="Sil" negative="Kapat" persistent>
    <span class="q-ml-sm">
      <p>{{ dataItem.teamName }} ekibinin silinmesini onaylıyor musunuz ?</p>
    </span>
  </form-modal-dialog>
  <!-- Delete modal end -->
</template>
...
import data from 'src/data/teamGuide.json'
